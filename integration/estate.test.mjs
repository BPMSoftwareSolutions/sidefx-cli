...
test('live estate: discovery, three plan formats, canonical invocation, proof and retained evidence',
  { skip: !estateRoot, timeout: 180_000 }, async t => {
    ...
    const list = await sdk.execute({ object: 'capability', verb: 'list' });
    ...
    const invocation = await sdk.execute({ object: 'capability', verb: 'invoke', subject: capabilityId, input });
    ...
    const receipt = await sdk.execute({ object: 'execution', verb: 'observe', subject: invocation.executionId });
    ...
    const cli = spawnSync(process.execPath, [bin, 'execution', 'observe', invocation.executionId, '--state', stateRoot, '--json'], { encoding: 'utf8' });
    assert.equal(cli.status, 0, cli.stderr);
    assert.deepEqual(JSON.parse(cli.stdout), receipt, 'CLI and SDK expose the same retained evidence');
    const proof = await sdk.execute({ object: 'capability', verb: 'evaluate', subject: capabilityId });
